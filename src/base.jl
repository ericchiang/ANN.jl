...
function sigm_derv(a::Vector{Float64})
    s_a = sigm(a)
    s_a .* (fill(1., size(s_a)) - s_a)
end

function NeuralLayer(in_dim::Int64, out_dim::Int64)
    # Glorot & Bengio, 2010
    b = sqrt(6) / sqrt(in_dim + out_dim)
    NeuralLayer(2b * rand(out_dim, in_dim) - fill(b, (out_dim, in_dim)),
                zeros(out_dim),
                sigm,
                sigm_derv,
...
mutable struct ArtificialNeuralNetwork
...
function softmax(ann_output::Vector{Float64})
    ...
    ann_output = [exp(i) for i in ann_output]
    ann_output / sum(ann_output)
end

function ArtificialNeuralNetwork(n_hidden_units::Int64)
    ann = ArtificialNeuralNetwork(Array{NeuralLayer}(undef, 1), [20], Array{Int64}(undef, 1))
end
...
function init!(ann::ArtificialNeuralNetwork,
                     x::Matrix{Float64},
                     y::Vector{Int64})
    layers = Array{NeuralLayer}(undef, length(ann.hidden_sizes) + 1)
...
function fit!(ann::ArtificialNeuralNetwork,
    ...
    for _ = 1:epochs
        for i = 1:n_obs
            ...
            y_hat[findfirst(isequal(y[i]),ann.classes)] = 1.
